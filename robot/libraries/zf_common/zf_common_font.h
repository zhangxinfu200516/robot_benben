...
typedef struct ChineseFont{
    uint8_t index[2];  // GB2312编码
    uint8_t bitmap[32]; // 点阵数据 (16列x2字节)
} ChineseFont;
...
// 自定义字库（只包含需要的汉字）
extern const ChineseFont custom_font[];
extern const ChineseFont chinese_font_16x16[];
extern const ChineseFont chinese_font_16x16[];
extern const uint16_t CHINESE_FONT_COUNT;
